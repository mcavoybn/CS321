...
public class Test {
	...
	public static void main(String[] args){
		if(args[0].equals("1")) {
			...
			File f = new File(System.getProperty("user.dir") + "/src/HW1/" + args[2]); //this needs to be fixed		
		...
		if(args[0].equals("2")) {
			...
			File f = new File(System.getProperty("user.dir") + "/src/HW1/" + args[3]); //this needs to be fixed		
			getCacheStats(cache, f);
		}			
	}	
	
	public static void getCacheStats(Cache<String> c, File f) {
		Scanner in;
		try {
			in = new Scanner(f);
			...
			while(in.hasNext()) {
				c.getObject(in.next());
				counter++;
				if(counter%50000==0 && counter<1000000) System.out.print('.');
			}
			System.out.println(' ');
			in.close();
